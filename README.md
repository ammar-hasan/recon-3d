...
Calibrated azimuths can be supplied by repeating `--view-azimuth` once per image. Full perspective calibration can instead be supplied by repeating `--camera-json` once per image; each JSON contains `focal_length_px`, `principal_point_px`, `camera_matrix_world`, and `look_at_target`. With at least two calibrated silhouettes, the pipeline voxel-carves a shared visual-hull mesh and keeps the original semantic construction parts as hidden editable guides. Full calibration uses exact perspective projection in the primary-camera frame; azimuth-only input retains the explicitly recorded orthographic assumption. If an exact hull fails the observed-view acceptance gates, the pipeline records the rejection and conservatively retries the supplied azimuth calibration. Manufactured-object axial/planar completion is recorded as a low-confidence generated hypothesis and can be disabled for ablation. The held-out evaluator uses the same exact camera-frame transform when available, renders an unused view without target-side alignment, and measures aligned surface Chamfer and normal consistency from the reference GLB. See [the calibrated multiview report](MULTIVIEW_REPORT.md).
